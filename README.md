...
## Открытые материалы и пересчёт

Этот репозиторий содержит не только выводы, но и обезличенные данные, из
которых основные числа пересчитываются заново:

- восстановление целостности V3;
- 35 строк normal-runtime conflict characterization;
- измерения plasticity и economics;
- 24 AST и проверку их принадлежности известному fixed-chain пространству.

```bash
python3 verify_public_evidence.py
```

Verifier использует только Python standard library, не доверяет итоговым
статусам как входу и не обращается к приватному коду или сети.

[Evidence ledger](docs/EVIDENCE_LEDGER.md) ·
[описание открытых данных](evidence/README.md) ·
[метод и ограничения](docs/08_METHOD_AND_LIMITATIONS.md)
